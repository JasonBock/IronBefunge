...
public static class ExecutorTests
{
	...
	public static void CreateWithNullRandomizer()
	{
		...
		Assert.That(() => new Executor([], reader, writer, (null as RandomNumberGenerator)!),
	...
	public static void Execute()
	{
		...
		var executor = new Executor(
			new Parser([">  @"]).Parse(),
			reader, writer);
		Assert.That(executor.Execute(), Is.Zero);
	}
	...
	public static void ExecuteWithRandomWhenTraceIsNull()
	{
		...
		Assert.That(() => new Executor(
			new Parser([">  @"]).Parse(),
			reader, writer, (null as RandomNumberGenerator)!), Throws.TypeOf<ArgumentNullException>());
	}
